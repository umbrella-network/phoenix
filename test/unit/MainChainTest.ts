...
import Chain from '../../artifacts/contracts/MainChain.sol/MainChain.json';
import IERC721 from '../../artifacts/contracts/i721.sol/i721.json';
...
const setup = async (requiredSignatures = 1) => {
  ...
  const iFactory = new ContractFactory(IERC721.abi, IERC721.bytecode, owner);
  ...
  // await contractRegistry.mock.getAddress.withArgs(toBytes32('HomeGate')).returns(stakingBank.address);

  const ierc721 = await iFactory.deploy('i token', 'it');
  await ierc721.deployed();

  const contract = await contractFactory.deploy(contractRegistry.address, timePadding, requiredSignatures);
  await contract.deployed();

  await contract.setToken(ierc721.address);
  await contract.setHomeGate(ierc721.address);
